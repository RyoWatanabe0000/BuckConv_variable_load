...
MINIBATCH_SIZE = 32
LEARNING_RATE = 4e-4
SAVE_DIR = f'agents64_4e-4_gauss040_without_steadyflag'
START_FLAG = True
break_flag = False
RESISTANCE_CHANGE_RATE = 200000
...
def save_status_graph_and_data_to_csv(action_list:list, status_list:list, status_list_smooth:list, SAVE_PATH):
    ...
    h2, l2 = ax2.get_legend_handles_labels()
    ax1.legend(h1 + h2, l1 + l2, loc='lower right')
    fig.savefig(f"{SAVE_PATH}/current_graph_smooth.png", bbox_inches="tight", pad_inches=0.05, dpi=600)
    plt.close()
...
def main():
    ...
    def eval_model():
        with agent.eval_mode():
            ...
            while True:
                if t == 100:
                    # 負荷変動できなかったらエラー
                    assert env_for_test.set_load_value(5)
    ...
    for episode in range(1, N_EPISODE + 1):
        ...
        while True:
            if step == load_change_index:
                # 負荷変動不可の場合エラー
                assert env.set_load_value(second_load_value)
        ...
        if episode % 10 == 0:
            ...
            #     pass
            # よい評価だった場合、その時点で探索の標準偏差を下げる
            if eval_reward_list[-1][1] > 140:
                agent.explorer.scale = 0.2
    ...
    with agent.eval_mode():
        ...
        while True:
            if t == 100:
                # print("load change method is called", end="")
                assert env_for_test.set_load_value(5)
